...
搜索链路:
  用户输入 → 300ms debounce
    → Tauri invoke("note_search", { query, limit: 20 })
      → Rust: SQLite FTS5 查询 (< 100ms for 10000 docs)
        → v1.1 可选: Tantivy 增强中文分词和排序
    ← 返回 TOP 20 结果
  → 渲染结果列表 (< 50ms)
═══════════════════════════════════
  总搜索延迟                     < 350ms
...
Web 端不进入桌面 MVP。它是桌面核心体验被验证后的后续路线，目标不是削弱版，而是提供和桌面端一致的核心能力：今日仪表盘、笔记、任务、日历、快速捕获、智能关联和搜索。差异在于 Web 端更强调跨设备访问、轻量部署和可选同步；桌面端继续承担本地文件系统、全局快捷键、离线体验和原生性能优势。
...
后续 Web 版本的原则：功能同构、体验近似、能力分层。核心页面和业务模型复用桌面端设计，但不强求 Web 端直接访问用户本地 Markdown 文件。
...
├──────────────────────────────────────────────────┤
│                  API Layer                         │
│  REST / tRPC / GraphQL (优先 REST + OpenAPI)       │
...
| 队列 | 首版可不用 | BullMQ / Cloudflare Queues 做索引和同步任务 |

推荐首个 Web 版本采用单体后端：一个 API 服务覆盖笔记、任务、日历、快速捕获、搜索和关联建议。等团队协作、同步和全文搜索压力上来后，再拆分服务。
...
| search_index | PostgreSQL FTS | 首版够用，数据增长后独立搜索服务 |
| attachments | Object Storage | 首版可先禁用或限制图片上传 |

同步分三阶段：

| 阶段 | 能力 | 策略 |
|------|------|------|
| Web 0.1 | Web 独立使用 | Web 数据在云端，支持 Markdown 导入/导出 |
| Web 0.2 | 桌面登录云账号 | 桌面端把本地数据增量上传到云工作区 |
...
| 领域 | 方案 |
|------|------|
| 认证 | Email magic link + OAuth，首版避免密码系统复杂度 |
...
首个 Web 版本推荐支持“弱离线”：已经打开的页面可继续查看和编辑最近内容，网络恢复后自动提交。
...
| 方案 | 推荐阶段 | 说明 |
|------|----------|------|
| Vercel + Neon/Supabase | Web 原型 | 最快上线，前后端开发效率高 |
...
Web 原型推荐：`apps/web` 部署到 Vercel，API 使用同仓库 server routes 或独立 `apps/api`，数据库使用 Neon/Supabase Postgres。
...
| 9 | 搜索先用 SQLite FTS5，Tantivy 作为增强候选 | 先控制 MVP 复杂度，中文分词质量不足时再升级 | 2026-05-20 |
| 10 | 增加 Web 应用形态 | 免安装跨设备访问，作为桌面端之外的轻量入口和未来协作基础 | 2026-05-19 |
| 11 | Web 端延后到桌面核心体验验证后 | 浏览器不能可靠管理本地 Markdown 文件，过早推进会稀释本地优先验证 | 2026-05-20 |
...
| 14 | v1 先做桌面核心闭环，不同时推进 Web/iOS/知识地图 | 避免范围蔓延，先验证“快捕获 + 本地笔记任务 + 今日视图”的核心价值 | 2026-05-20 |
| 15 | v1 搜索先用 SQLite FTS5，Tantivy 作为验证项 | 降低首版复杂度，只有中文搜索质量不足时再引入独立索引 | 2026-05-20 |

---

## 附录 A：技术预研待验证项

Phase 0 只回答“能不能用这套技术做出足够快、足够稳的核心体验”。每个验证项都要产出可运行 demo、测量结果和是否进入 MVP 的结论。

| # | 验证项 | 方法 | 通过标准 |
|---|--------|------|----------|
| 1 | Tauri v2 冷启动实际耗时 | 创建空项目，打 release build 测 10 次取中位数 | 主窗口可交互时间 < 1s |
| 2 | Quick Capture 独立窗口 | 全局快捷键唤起 500px 浮动窗口，输入后写入本地数据 | 唤起 < 300ms，写入成功有明确反馈 |
| 3 | SQLite + Markdown 双写一致性 | 创建/编辑/删除 20 条笔记和任务，重启后重建索引 | Markdown 可读，SQLite 可重建，无孤儿记录 |
| 4 | Tiptap 自定义节点 | 写一个 `TaskNode`，在编辑器内渲染任务并能切换完成状态 | 状态能同步到任务表，刷新后保持 |
| 5 | SQLite FTS5 中文搜索 | 索引 1000 篇中文 Markdown，测标题/正文/标签搜索 | 常见中文关键词结果可接受，响应 < 100ms |
| 6 | Tantivy 是否必要 | 同一批数据用 Tantivy 建索引，对比 FTS5 的准确率和复杂度 | 只有体验明显更好才进入 v1.1 |
| 7 | WebView 渲染差异 | Windows WebView2 和 macOS WKWebView 跑同一 UI 原型截图 | 核心布局无明显错位 |

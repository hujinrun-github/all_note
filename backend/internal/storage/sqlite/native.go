...
func (r voiceNoteRepository) ClaimUpload(ctx context.Context, clientID string, claim model.VoiceUploadClaim) (*model.VoiceNote, error) {
	...
	result, err := r.db.ExecContext(ctx, `
		UPDATE voice_notes
		SET object_key = CASE WHEN upload_state = 'uploaded' THEN object_key ELSE ? END,
			mime_type = CASE WHEN upload_state = 'uploaded' THEN mime_type ELSE ? END,
			audio_size = CASE WHEN upload_state = 'uploaded' THEN audio_size ELSE ? END,
			audio_sha256 = CASE WHEN upload_state = 'uploaded' THEN audio_sha256 ELSE ? END,
...
func (r voiceNoteRepository) markUploaded(ctx context.Context, clientID, sha256 string) (*model.VoiceNote, error) {
	...
	now := time.Now().Unix()
	result, err := r.db.ExecContext(ctx, `
		UPDATE voice_notes SET upload_state = 'uploaded', updated_at = ?
		WHERE workspace_id = ? AND client_id = ? AND audio_sha256 = ?
...
func (r voiceNoteRepository) MarkUploadFailed(ctx context.Context, clientID, sha256 string) error {
	...
	_, err = r.db.ExecContext(ctx, `
		UPDATE voice_notes SET upload_state = 'failed', updated_at = ?
		WHERE workspace_id = ? AND client_id = ? AND audio_sha256 = ? AND upload_state <> 'uploaded'
...
func (r voiceNoteRepository) ListRecent(ctx context.Context, limit int) ([]model.VoiceNote, error) {
	...
	rows, err := r.db.QueryContext(ctx, sqliteVoiceNoteSelect+`
		WHERE v.workspace_id = ? ORDER BY v.updated_at DESC LIMIT ?
	`, workspaceID, limit)
	if err != nil {
		return nil, err
	}

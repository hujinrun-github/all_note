...
func (s *store) LoadTaskDomainRuntimeState(ctx context.Context, workspaceID string) (storage.TaskDomainRuntimeState, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT domain.workspace_id,domain.model_version,domain.migration_state,
		...
		WHERE domain.workspace_id=$1`, workspaceID).Scan(
...
func (r *postgresTaskDomainV2ProjectReader) GetTaskAggregateState(ctx context.Context, taskID string) (taskdomain.TaskAggregateState, error) {
...
var _ taskdomain.TaskAggregateStateReader = (*postgresTaskDomainV2ProjectReader)(nil)

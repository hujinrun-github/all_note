...
func getPostgresRoadmap(ctx context.Context, q postgresTaskDomainV2Queryer, w, column, value string) (taskdomain.RoadmapSnapshot, error) {
	...
	// A transaction owns one PostgreSQL connection. Finish consuming and close
	// the node cursor before issuing per-node progress queries on that same
	// connection.
	for index := range out.Nodes {
		out.Nodes[index].Progress, err = postgresRoadmapProgress(ctx, q, w, out.Nodes[index].Node.ID)
		if err != nil {
			return out, err
		}
	}

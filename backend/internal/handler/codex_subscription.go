...
type CodexSubscriptionService interface {
	...
	Poll(context.Context, string, string, string, int64) (codexsubscription.PollResult, error)
}
...
func PollCodexSubscription(service CodexSubscriptionService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, err := service.Poll(c.Request.Context(), identity.UserID, identity.WorkspaceID, flowID, request.ExpectedRevision)
		if err != nil {

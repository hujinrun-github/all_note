...
export type NoteAttachmentKind = 'audio' | 'video' | 'image' | 'file'
...
export async function getNote(id: string) {
  const res = await api.get<{ note: Note }>(
...
export async function getNoteAttachments(noteID: string) {
  const response = await api.get<{ attachments: NoteAttachment[] }>(
    `/api/notes/${encodeURIComponent(noteID)}/attachments`
  )
  return response.data.attachments
}

export async function uploadNoteAttachment(noteID: string, file: File) {
  const response = await api.postBlob<{ attachment: NoteAttachment }>(
    `/api/notes/${encodeURIComponent(noteID)}/attachments`,
    file,
